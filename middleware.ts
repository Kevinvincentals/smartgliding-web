...
export async function middleware(request: NextRequest) {
  ...
  // Handle /api/tablet/* and /api/reports/* routes (Cookie-based authentication)
  if (pathname.startsWith('/api/tablet/') || pathname.startsWith('/api/reports/')) {
  ...
  // If not an /api/tablet/ route, fall through to existing/other API auth logic (Bearer token based)
  // This combines the original publicRoutes logic, excluding the tablet ones already handled.
  const combinedPublicRoutes = [...generalAuthPublicRoutes, ...tabletAuthPublicRoutes]
